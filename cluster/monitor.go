...
// 对stash每个环节加以监控，数据最好本地持久化，保持严谨
// 除了对prometheus友好，也要有自己一套业务监控
var (
	prefix = "data_stash_"
...
// StartHttp 开启prometheus http服务
func StartHttp(prometheusAddr string){
	...
	goglog.Logger.Fatal(http.ListenAndServe(prometheusAddr, nil))
}
	...
	for {
		...
		case <-ticker.C:
			if err := p.pusher.Push(); err != nil {
				...
				goglog.Logger.Error("pushing metrics failed", zap.Error(err))
				p.reconnect()
			}
		case <-p.ctx.Done():
			goglog.Logger.Info("Pusher.Run quit due to context has been canceled")
			break FOR
		}
	}
	p.stopped <- struct{}{}
...
func (p *Pusher) Stop() {
	...
	if err := p.pusher.Delete(); err != nil {
		err = errors.Wrapf(err, "")
		goglog.Logger.Error("failed to delete metric group", zap.String("pushgateway", p.pgwAddrs[p.inUseAddr]),
			zap.String("job", "clickhouse_sinker"), zap.String("instance", p.instance), zap.Error(err))
	}
	goglog.Logger.Info("stopped metric pusher")
}
